...
trait WritingTransformer[W: CanBeWritten, M[+ _]: Computation]
  ...
  override private[pdbp] def liftComputation[Z](mz: M[Z]): WTM[Z] = {
    bindM(mz, { z => resultM((empty, z)) })
  }

  override private[pdbp] def bind[Z, Y](wtmz: WTM[Z], `z=>wtmy`: Z => WTM[Y]): WTM[Y] =
    bindM(wtmz, { (w1, z) => 
  ...
  override def execute(`u>=wtk=>u`: Unit `>=WTK=>` Unit): Environment `I=>` Unit = { implicit environment =>
    executeK { u: Unit =>
      bindM(`u>=wtk=>u`(u), { case (_, u) =>
        resultM(u)
      })
    }
  }
